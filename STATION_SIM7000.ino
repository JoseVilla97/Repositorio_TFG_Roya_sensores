...
#define TINY_GSM_MODEM_SIM800                //Se define la placa
...
#define DEVICE_ID "BME1"    //Thinger device IC
#define DEVICE_CREDENTIAL "AVr_6%lvo-Q0pOGf" //Thinger device credential (password)
...
#define VBAT_PIN             35 //Pin de lectura de voltaje  //Definimos los pines para leer el voltaje de la bateria como método de aproximación
#define Soil_PIN             14 //Pin de lectura del sensor de humedad del suelo
#define DS3231_I2C_ADDRESS 0x57 //Según el escaneo de direcciones para el DS3231 tenemos Addresses 0x57, 0x68
...
//#define PIN_SENSORBME_POWER 35 // Pin donde está conectado el sensor BME280
//#define PIN_SENSORSOIL_POWER 14
...
const int button = 12;             //gpio to use to trigger delay //que se usa para simular un bloqueo al presionar un botón
const int wdtTimeout = 60000;     //time in ms to trigger the watchdog// tiempo en el que se quedó pegado por unos 20s
...
//Variables del sensor de humedad del suelo //Estos valores salen de la calibración en laboratorio según las lecturas vistas en las muestras del suelo de la finca
const float dry = 1.90; // Voltage value for dry soil
const float wet = 0.90; // Voltage value for wet soil
...
void setup() {
  ...
  //  pinMode(PIN_SENSORSOIL_POWER, OUTPUT); // Configura el pin del sensor como salida
  //  digitalWrite(PIN_SENSORSOIL_POWER, LOW);
  //  Serial.println("Prendiendo sensores");
  //  digitalWrite(PIN_SENSORSOIL_POWER, HIGH); // Enciende el sensor al principio del program
  ...
  //rtc.setI2CAddress(DS3231_I2C_ADDRESS);
  if (!rtc.begin()) {
  ...
  if (!file) {
    ...
    writeFile(SD, "/Datos.csv", "fecha, Temperature, Humidity, DewPoint, Dewpoint Ajustado, Contenido volumetrico %, Soil moisture map %, Pressure (Pa), Altitud (m), Bits \r\n");
  }
  else {
...
void deepSleep1() {
  ...
  //  digitalWrite(PIN_SENSORSOIL_POWER, LOW); // Enciende el sensor al principio del program
...
void Sensor() { // Función que se ejecuta periodicamente con el timer1
  ...
  dataMessage = timeString + ", " + String(temperature_SET) + ", " + String(humidity_SET) + ", " + String(DewPoint) + ", " + String(DewPoint_Ajustado) + ", " + String(Contenido_Vol_real_Porcent) + ", " + String(soil_value) + ", " + String(percent_Humididy) + ", " + String(pressure) + ", " + String(altitud) + ", " + String(soil_value) + "\r\n";
