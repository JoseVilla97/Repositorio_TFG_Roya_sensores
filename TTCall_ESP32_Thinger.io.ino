...
#define VBAT_PIN 35 //Pin de lectura para estimar el voltage 
float bat_mv = 0;
float batteryLevel;
int analogValor;
float voltage;
...
void loop() {
  ...
  if (readCount >= 3) {//Nose porque pero ocupo almenos 2 corridas para que la plataforma tenga tiempo de subir los datos de la primera corrida
...
void Datos() { // Función que se ejecuta periodicamente con el timer1
  ...
  Serial.print("Voltage: "); //Método numero 1
  vBat = leerVoltaje(VBAT_PIN);
  Serial.print(vBat, 1);
  Serial.println(" (V)");

  getBatteryFromADC(); //Método numero 2
  Serial.println("");
  ...
  Serial.println("");
  if (readCount == 2) { //queremos enviar la segunda corrida no la primera
    enviarDatos = true;
  }
}
